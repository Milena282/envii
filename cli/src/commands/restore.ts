...
import { scanDirectory, buildFingerprintMap, Project } from '../core/scanner.js';
...
export async function restoreCommand(options: RestoreOptions): Promise<void> {
  ...
  try {
    ...
    scanSpinner.succeed(`Found ${matched.length} matching project${matched.length === 1 ? '' : 's'}`);
    ...
    if (matched.length === 0) {
      ...
      logger.info('Make sure you have cloned/created the projects before restoring.');
