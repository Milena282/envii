...
export async function deriveKey(phrase: string, salt: string): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    crypto.pbkdf2(phrase, saltBuffer, PBKDF2_ITERATIONS, KEY_LENGTH, 'sha256', (err, key) => {
      if (err) reject(err);
      else resolve(key);
    });
  });
 ...
 */
export async function encryptBackup(data: string, key: Buffer): Promise<string> {
...
/**
 * Full decryption pipeline: base64 → decrypt → decompress
 */
export async function decryptBackup(encryptedBase64: string, key: Buffer): Promise<string> {
 ...
 */
export async function getSizes(data: string): Promise<{ original: number; compressed: number }> {
